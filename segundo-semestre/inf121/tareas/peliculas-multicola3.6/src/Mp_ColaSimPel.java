...
public class Mp_ColaSimPel {
  private int n;
  private ColaSimPel c[] = new ColaSimPel[100];

  public Mp_ColaSimPel() {
  ...
  // Adicionar un element (De tipe Pelicula) en la pila en la posicion "i"
  void adicionar(int i, Pelicula e) {
    if (!c[i].esLlena()) {
      c[i].adi(e);
      if (i > n)
        n = i;
  ...
  // Eliminar el elemento (De tipe Pelicula) en la pila con posicion "i"
  Pelicula eliminar(int i) {
    Pelicula e = new Pelicula("", "", 0, 0);
    if (!c[i].esVacia())
      e = c[i].eli();
    else
  ...
  void vaciar(int i, ColaSimPel z) { c[i].vaciar(z); }
